...
export class ConsumptionHistoryController {
  ...
  async statisticsByUser(@Param('userId') userId: string, @User() user: JwtUser) {
    const data = await this.consumptionService.statisticsByUser(+userId, user);
    return buildSuccessResponse(ResponseCode.C00185, data);
  }
  ...
  async statisticsByFamily(@Param('familyId') familyId: string, @User() user: JwtUser) {
    const data = await this.consumptionService.statisticsByFamily(+familyId, user);
    return buildSuccessResponse(ResponseCode.C00186, data);
  }
}
