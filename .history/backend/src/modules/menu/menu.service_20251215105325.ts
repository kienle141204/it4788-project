...
export class MenuService {
  ...
  async findOne(id: number, userId: number, userRole: string): Promise<Menu> {
    ...
    if (!menu) {
      throw new NotFoundException(ResponseMessageVi[ResponseCode.C00130]);
    }
  ...
  async createMenu(createMenuDto: CreateMenuDto, familyId: number, userId: number, userRole: string): Promise<Menu> {
    ...
    if (!family) {
      throw new NotFoundException(ResponseMessageVi[ResponseCode.C00143]);
    }
    ...
    if (userRole === 'user') {
      ...
      if (!isMember && family.owner_id !== userId) {
        throw new ForbiddenException(ResponseMessageVi[ResponseCode.C00138]);
      }
    }
  ...
  async addDishToMenu(
    ...
    if (existingMenuDish) {
      throw new ForbiddenException(ResponseMessageVi[ResponseCode.C00141]);
    }
  ...
  async updateMenuDish(
    ...
    if (!menuDish) {
      throw new NotFoundException(ResponseMessageVi[ResponseCode.C00142]);
    }
