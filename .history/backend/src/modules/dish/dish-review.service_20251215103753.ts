...
export class DishReviewService {
  ...
  async createReview(
    ...
    if (existingReview) {
      throw new ConflictException(ResponseMessageVi[ResponseCode.C00156]);
    }
  ...
  async updateReview(
    ...
    if (!review) {
      throw new NotFoundException(ResponseMessageVi[ResponseCode.C00150]);
    }

    if (review.user_id !== userId) {
      throw new ForbiddenException(ResponseMessageVi[ResponseCode.C00157]);
    }
  ...
  async deleteReview(reviewId: number, userId: number): Promise<void> {
    ...
    if (review.user_id !== userId) {
      throw new ForbiddenException(ResponseMessageVi[ResponseCode.C00158]);
    }
