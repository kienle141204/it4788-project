...
export class FamilyController {
  ...
  async getAllFamilies() {
    const data = await this.familyService.getAllFamilies();
    return buildSuccessResponse(ResponseCode.C00201, data);
  }

  @Get('my-family')
  @ApiOperation({ summary: 'Lấy ra các nhóm gia đình có người dùng là thành viên' })
  async getMyFamily(@User() user: JwtUser) {
    const data = await this.familyService.getMyFamily(user.id);
    return buildSuccessResponse(ResponseCode.C00207, data);
  }
  ...
  async getFamilyMembers(
    ...
    const data = await this.familyService.getFamilyMembersWithDetails(id, user.id);
    return buildSuccessResponse(ResponseCode.C00203, data);
  }
  ...
  async getInvitationCode(
    ...
    const data = await this.familyService.getInvitationCode(id, user.id, user.role);
    return buildSuccessResponse(ResponseCode.C00204, data);
  }
  ...
  async getFamilyById(@Param('id', ParseIntPipe) id: number) {
    const data = await this.familyService.getFamilyById(id);
    return buildSuccessResponse(ResponseCode.C00202, data);
  }
  ...
  async updateFamily(
    ...
    const data = await this.familyService.updateFamily(id, dto, user.id, user.role);
    return buildSuccessResponse(ResponseCode.C00205, data);
  }
  ...
  async deleteFamily(
    ...
    return buildSuccessResponse(ResponseCode.C00206, { family_id: id });
  }
}