  ...
  async removeMemberById(id: number) {
    ...
    try {
      ...
      for (const memberItem of remainingMembers) {
        ...
        console.log(`[removeMemberById] Notification title: "Thành viên rời nhóm"`);
        console.log(`[removeMemberById] Notification body: "${removedUserName} đã rời nhóm ${family.name}"`);
        // Gửi thông báo cho các thành viên còn lại
        try {
          const notification = await this.notificationsService.createNotification(
            memberItem.user_id,
          ...
          console.error(`[removeMemberById] ❌ Failed to create notification for user ${memberItem.user_id}:`, notifError);
          throw notifError; // Re-throw để xem lỗi chi tiết
        }
      }
