...
export class MemberService {
  ...
  async addMember(
    ...
    if (family.owner_id !== user.id && user.role !== 'admin') {
      throw new ForbiddenException(ResponseMessageVi[ResponseCode.C00211]);
    }

    const exists = await this.memberRepository.findOne({
      where: { family_id, user_id },
    });
    if (exists) throw new BadRequestException(ResponseMessageVi[ResponseCode.C00212]);
  ...
  async getMember(id: number) {
    ...
    if (!member) throw new NotFoundException(ResponseMessageVi[ResponseCode.C00213]);
    return member;
  }

  async getMyFamily(userId: number) {
    const member = await this.memberRepository.find({ where: { user_id: userId } });
    if (member.length === 0) throw new NotFoundException(ResponseMessageVi[ResponseCode.C00214]);
    return member;
  }
  ...
  async updateMemberRole(
    ...
    if (!family) throw new NotFoundException(ResponseMessageVi[ResponseCode.C00215]);

    if (family.owner_id !== user.id && user.role !== 'admin') {
      throw new ForbiddenException(ResponseMessageVi[ResponseCode.C00216]);
    }
  ...
  async removeMember(id: number, user: JwtUser) {
    ...
    if (family.owner_id !== user.id && user.role !== 'admin') {
      throw new ForbiddenException(ResponseMessageVi[ResponseCode.C00217]);
    }
